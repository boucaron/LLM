#!/bin/bash
#
# Shared llama.cpp benchmark runner (macOS / MacBook Air M4).
#
# Usage:
#   ./bench.sh <config>        e.g. ./bench.sh Ornith-1-9B-Q4_K_M
#
# Per-model settings live in scripts/configs/<config>.sh and set:
#   MODEL_FILE     GGUF filename, relative to the parent directory
#   SERVER_FLAGS   flags shown in the "Start:" hint (array)
#   LLAMA_SERVER   optional override of the llama-server binary path
#   PROMPTS        optional override of the benchmark prompts
...
if [[ $# -ne 1 ]]; then
    ...
    ls "$SCRIPT_DIR"/configs/*.sh 2>/dev/null \
        | xargs -n1 basename \
        | sed 's/\.sh$//' \
        | sed 's/^/  /'
    exit 1
...
LLAMA_SERVER="${LLAMA_SERVER:-$SCRIPT_DIR/llama-server}"
...
FLAGS_STR=$(printf '%s ' "${SERVER_FLAGS[@]}")
...
$LLAMA_SERVER -m \$MODEL $FLAGS_STR
"
